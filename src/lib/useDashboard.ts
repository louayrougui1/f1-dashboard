...
export function useDashboard(
  initial: DashboardInitial = { season: null, round: null },
): DashboardState {
  ...
  const [slices, setSlices] =
    useState<Record<DataKey, SliceState<unknown>>>(EMPTY);
  ...
  const cacheKey = useCallback(
    (k: DataKey, season: string, r: number | null): string => {
      if (
        k === "results" ||
        k === "qualifying" ||
        k === "pitStops" ||
  ...
  const calendar = useMemo(() => {
    const data = slices.schedule.data as Race[] | null;
    return data ?? EMPTY_RACES;
  }, [slices.schedule]);

  const completedRaces = useMemo(
    () =>
      calendar.filter(
        (r) => r.start !== null && r.start.getTime() <= now.getTime(),
      ),
    [calendar, now],
  );
  ...
  const scheduleReady =
    slices.schedule.status === "ready" && calendar.length > 0;
  const featuredRound = useMemo(() => {
    if (round !== null) {
      if (!scheduleReady) return null;
      return calendar.some((r) => r.round === round) ? round : null;
    }
    return scheduleReady ? (lastRace?.round ?? null) : null;
  }, [round, scheduleReady, calendar, lastRace]);

  // If a carried round is not present in the loaded season calendar, follow latest.
  useEffect(() => {
    if (!scheduleReady || round === null) return;
    if (!calendar.some((r) => r.round === round)) {
      setRound(null);
    }
  }, [scheduleReady, calendar, round]);
  ...
  useEffect(() => {
    ...
    const isStaleRun = () =>
      controller.signal.aborted || runId !== effectRunId.current;
    ...
    const defs: Array<{
      ...
      {
        key: "schedule",
        cache: keyFor("schedule"),
        run: (s) => fetchSchedule(seasonId, s),
      },
      {
        key: "drivers",
        cache: keyFor("drivers"),
        run: (s) => fetchDriverStandings(seasonId, s),
      },
      {
        key: "constructors",
    ...
    if (featuredRound !== null && scheduleReady && !featuredUpcoming) {
      ...
      if (featuredRace?.weekend.sprint != null) {
        ...
        setSlices((prev) => ({
          ...prev,
          sprint: { status: "ready", data: null, error: null },
        }));
      }
      defs.push({
        key: "laps",
    ...
    for (const f of defs) {
      ...
      if (cached !== null) {
        setSlices((prev) => ({
          ...prev,
          [f.key]: { status: "ready", data: cached, error: null },
        }));
        completed += 1;
        if (!isStaleRun() && completed === total) setRefreshing(false);
        continue;
      }
      const start = () => {
        f.run(controller.signal)
          .then((value) => {
            ...
            setSlices((prev) => ({
              ...prev,
              [f.key]: { status: "ready", data: value, error: null },
            }));
          ...
          .catch((err: unknown) => {
            ...
            setSlices((prev) => {
              ...
              return {
                ...prev,
                [f.key]: { status: "error", data: null, error },
              };
            });
          })
          .finally(() => {
    ...
    if (total === 0 && !isStaleRun()) setRefreshing(false);
  ...
  const setSeason = useCallback(
    (season: string) => {
      const nextSeason = season === "current" ? "current" : season;
      if (seasonId === nextSeason) return;
      setRound(null);
  ...
  return {
    ...
    constructorStandings: slices.constructors as SliceState<
      ConstructorStandingRow[]
    >,
