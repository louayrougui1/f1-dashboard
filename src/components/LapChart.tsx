...
function isSeries(value: Series | null): value is Series {
  return value !== null;
}
...
export function LapChart({
  ...
  const chart = useMemo(() => {
    ...
    const y = (pos: number) =>
      PAD.top + (pos - 1) * (innerH / Math.max(1, maxPos - 1));
    ...
    const series: Series[] = selected
      .map((s): Series | null => {
        ...
        return {
          id: s.id,
          code: s.row
            ? driverCode(s.row.driver)
            : s.id.toUpperCase().slice(0, 3),
          color: s.row
            ? teamColor(s.row.constructor.constructorId)
            : teamColor(null),
          segments,
          last,
          finalPos: s.lastPos,
    ...
    return (
      <div className="flex flex-col items-start gap-1 rounded-lg border border-line bg-surface px-4 py-6">
        ...
        <p className="text-xs text-muted">
          Lap-by-lap data has not been published yet.
        </p>
      </div>
    );
  }
  if (!detail || detail.laps.length === 0 || chart.series.length === 0) {
    return (
      <div className="flex flex-col items-start gap-1 rounded-lg border border-line bg-surface px-4 py-6">
        <p className="label text-text">Lap Chart</p>
        <p className="text-xs text-muted">
          Lap-by-lap data is not available for this round.
        </p>
      </div>
    );
  }
    ...
    <div className="rounded-lg border border-line bg-surface">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line bg-bg-secondary/50 px-4 py-2.5 lg:px-5">
        ...
        <p className="label text-[11px] text-muted/70">
          {raceName
            ? `${display(raceName)} · ${display(roundLabel(round))}`
            : "Position over race distance"}
        </p>
      </div>
      <div className="px-3 pt-3">
        <svg
          ...
          role="img"
          aria-label="Driver positions by lap"
        >
          {posLabels.map((p) => (
            <g key={`ph-${p}`}>
              <line
      ...
      <p className="px-4 pb-3 text-[11px] text-muted/80 lg:px-5">
        Top {series.length} classified drivers · Line ends where a driver
        retires
      </p>
    </div>
  );
